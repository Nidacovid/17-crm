...
## Fase 10 — Google Calendar y feed .ics (2026-09-16)

Integración con Google Calendar (D-12) y feed propio `.ics` (D-13). El flujo OAuth vive en `app/api/google/{connect,callback,disconnect}` con `access_type=offline` + `prompt=consent` y `state` en cookie httpOnly; los tokens se guardan cifrados con AES-256-GCM (`lib/google/crypto.ts`, `ENCRYPTION_KEY` generada) en `private.google_credentials`, y `lib/google/oauth.ts` expone `getGoogleClient` con refresco a menos de 5 minutos, borrado de credenciales ante `invalid_grant` y sin reintentos en bucle. `lib/google/calendar.ts` implementa `syncPayment` según la tabla de 10.5 (insert/patch/delete por estado, eventos de día completo desplazados `reminder_days_before`, popup a `reminder_time` desde medianoche con signo negativo, `extendedProperties.private` con `crm_payment_id`/`crm_project_id`, calendario dedicado `Cobros · CRM`), enganchado a las cinco Server Actions de pagos tras la escritura y envuelto en `try/catch` que nunca propaga errores; `POST /api/payments/resync` procesa pendientes y errores en serie con pausa de 200 ms. El feed `GET /api/calendar/[token]` (404 si el token no existe, cabeceras `text/calendar` y `max-age=900`, generación por plantillas `\r\n` con plegado a 75 octetos sin partir UTF-8) se construye en `lib/ics/build.ts` sin dependencias. La sección Google de `/ajustes` (10.8) queda completa (estado, conectar/desconectar, calendario con enlace, hora y días de antelación, resincronización con recuento, errores listados, URL `.ics` con copiar, regenerar token con advertencia y nota del modo Testing) y Home lee el calendario vía `GET /api/google/events` (calendario principal + Cobros, caché de 5 minutos, `connected:false` con 200) para `CalendarCard` y los "Eventos de hoy" de `TodayCard`.

### Decisiones de implementación

- **Los eventos del calendario de Cobros no se duplican en Home**: `CalendarCard` (10.4 de la fase anterior) ya pinta los vencimientos desde `v_payments` sin depender de Google, así que del endpoint `/api/google/events` solo se muestran los de `source: "principal"`; lo mismo en "Eventos de hoy". El endpoint sí devuelve ambos calendarios tal como exige 10.6.
- **Caché de 5 minutos en memoria** (`Map` con TTL en `listGoogleEvents`) en lugar de `revalidate` de segmento: la ruta lee cookies de sesión, es dinámica y el `revalidate` de Next 15 no aplica a handlers con cookies; el efecto sobre la cuota es el mismo y solo se cachean resultados con `connected: true`.
- **`TRIGGER;VALUE=DATE-TIME` absoluto (UTC) en el VALARM del `.ics`**: con eventos de día completo, los disparadores relativos se interpretan contra medianoche de forma distinta según el cliente (Apple vs RFC estricta); el absoluto a `reminder_time` en `Europe/Madrid` del día del evento es inequívoco. En el evento de Google se aplica literalmente `minutes = -(hora × 60 + minutos)` (10.5); el signo queda pendiente de verificar contra la respuesta real de la API al conectar la cuenta.
- **El `.ics` es espejo exacto del evento de Google**: mismo `SUMMARY` (`Cobro · {negocio} · {importe}` con `formatEUR`), misma descripción de tres líneas con `Abrir: {APP_URL}/proyectos/{id}` y el día también desplazado `reminder_days_before`, para que Google y Apple muestren el mismo día; `DTEND` omitido (evento de un día, como lista 10.7).
- **Borrados de evento tolerantes a 404/410** en `syncPayment`: si el evento ya no existe en Google (calendario borrado, token revocado y reconectado), el pago se limpia igualmente (`no_aplica`) en vez de quedar en `error` para siempre; otros errores sí se registran en `sync_error` para el botón de resincronización.
- **`reminder_time` se normaliza a `HH:MM` al leer** de Postgres (devuelve `HH:MM:SS`), tanto para Google, `.ics` como para el `input type="time"` de Ajustes; se reutiliza `settingsSchema.pick` para la validación de la acción (regla 2.4.7) y el token nuevo del feed se genera con `randomBytes(24).toString("hex")`, el mismo formato del default de la base.
- **`ENCRYPTION_KEY`** generada con `openssl rand -base64 32` (32 bytes verificados) y fijada en `.env.local`; nunca aparece en código ni logs, y ningún token se registra ni truncado (10.2).
- **`Conectar con Google` es un `Link` con `prefetch={false}`**: es una ruta API, no una página; sin eso el prefetch del router ejecutaría el handler sin clic del usuario.

### Verificación

- `npm run build`, `npm run lint` y `npx tsc --noEmit` sin errores ni advertencias.
- En runtime (servidor de desarrollo, con `/` y `/ajustes` temporales en las rutas públicas y revertido después): `GET /api/calendar/{token inválido}` → 404; `GET /api/calendar/{token real}` → 200 con `Content-Type: text/calendar; charset=utf-8`, `Cache-Control: public, max-age=900`, un `VEVENT` por cada pago pendiente (6 en la base real) con `UID pago-{id}@crm`, `DTSTART;VALUE=DATE`, `SUMMARY Cobro · GM PUBLICIDAD · 100\,00 €` (coma escapada), `DESCRIPTION` con `\n` literales y plegado de línea a 75 octetos, y `VALARM` absoluto `20260915T070000Z` (= 09:00 `Europe/Madrid`); `/ajustes` renderiza "No conectado", "Conectar con Google", "Resincronizar cobros con el calendario", el recuento de pendientes, los campos de aviso, la URL del feed con las instrucciones de Apple, "Regenerar token del feed" y la nota del modo Testing con la caducidad a 7 días; `/` renderiza Home sin errores.
- Sin sesión, `/api/google/events`, `/api/payments/resync` y `/api/google/connect` redirigen a `/login`; `/api/google/callback` con `state` inválido no canjea nada.
- Con la clave anónima, `private.google_credentials` es inaccesible: PostgREST responde 404 ("Could not find the table", el esquema `private` no está expuesto en la API) y además su RLS no tiene ninguna política para `authenticated` (Fase 1). Verificación literal con `set role anon` pendiente de acceso directo a la base.
- Pendiente de verificar con la cuenta real de Google (requiere el consentimiento del usuario): guardado de tokens cifrados y creación del calendario `Cobros · CRM`, creación/actualización/borrado de eventos desde las acciones de pago, detección del token revocado y el signo del `minutes` del aviso en la respuesta real de la API.

### Incidencia detectada en la primera conexión real (2026-09-16)

La primera conexión de Google "funcionó" en pantalla pero no guardó nada: los 6 pagos seguían `pendiente`, `google_calendar_id` era null y Home respondía `connected: false`. Causa raíz: **el esquema `private` no estaba expuesto en la API del proyecto Supabase**, así que PostgREST rechazaba con `PGRST106 Invalid schema: private` TODOS los accesos a `private.google_credentials` (también los de la clave de servicio), y el `upsert` de `saveTokens` ignoraba el error y devolvía éxito. Correcciones aplicadas al código: `saveTokens` lanza si el `upsert` falla (el callback redirige a `?google=error`), `readCredentials` trata un error de API como "sin credenciales", `deleteCredentials`/`revokeAndDeleteCredentials` informan del fallo en vez de callar, `syncPayment` crea el calendario `Cobros · CRM` si falta (auto-reparación antes del primer `events.insert`), y si `events.patch` encuentra 404/410 recrea el evento en vez de duplicarlo. Requisito operativo pendiente del usuario: **añadir `private` a los esquemas expuestos** (Dashboard → Settings → API → Exposed schemas); la seguridad de D-15 no cambia porque la RLS de `google_credentials` sigue sin políticas para `anon`/`authenticated` (con la clave anónima seguirá devolviendo vacío o denegado).
